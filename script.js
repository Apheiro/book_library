...
function fillEditForm(node) {
    const book = node.parentNode.parentNode;
    const data = bookCollection[nodeIndexItem(library, book)];
...
function openForm(e) {
    ...
    if (e.target.classList.contains('editButton')) {
        ...
        indexOfBook = nodeIndexItem(library, book);
...
function closeForm() {
    form.classList.add('animationFadeOut');
    editForm.classList.add('animationFadeOut');
    formBackground.classList.add('animationFadeOut');
    setTimeout(() => {
        form.classList.remove('animationFadeOut');
        editForm.classList.remove('animationFadeOut');
        formBackground.classList.remove('animationFadeOut');
...
function changeReadStatus() {
    const bookIndex = this.parentNode.parentNode.parentNode.getAttribute('data-index');
    const bookData = bookCollection[bookIndex];
    if (this.checked) { bookData.read = true; } else { bookData.read = false; };
}
...
function createBookCard(title, author, pages, read) {
    ...
    bookReadCheckbox.addEventListener('change', changeReadStatus)
...
function deleteBook() {
    ...
    book.classList.add('animationFadeOut');
    setTimeout(() => {
        bookCollection.splice(bookIndex, 1);
        book.remove();
        matchingIndex();
        if (!library.contains(book)) { insertBookBtnLibrary(); };
    }, 480);
}

function insertEditBookData() {
    const book = library.childNodes[indexOfBook]
...
colorInput.addEventListener('input', e => root.style = `--color: ${e.target.value}`);
colorInputEdit.addEventListener('input', e => root.style = `--color: ${e.target.value}`);